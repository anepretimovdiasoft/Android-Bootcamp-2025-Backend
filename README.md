...
## 2.Регистрация(пользователи) 
    ...
    5.Отображение волонтерских центров(пользователи, админы)
    6.Реализовать эндпоинт для получения списка волонтерских центров:
    Метод: GET api/search/groups/{user_cord}
    7.Написать сортировку центров по расстоянию до устройства

## 3.Отображение конкретного волонтерского центра(пользователи, админы)
...
## Пользователь
    ...
    3.email
    4.пароль
    5.group(Принадлежность к какой либо группе true или false)
    6.group_id(null - если group = false)
    7.avatar(url на аватарку) (опционально)
    8.bio(Описание пользователя)
## Админ
    1.id
    2.name
    3.email
...
## Волонтерский центр
    1.id
    2.name
    3.volunteers(Список волонтеров центра)
    4.avatar(url на аватарку) (опционально)
    5.bio(Описание центра)
