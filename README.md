...
- [ ] Обеспечить запуск процесса авторизации после успешной регистрации (вызов `AuthorizeUseCase`)
...
- [ ] Обеспечить поддержку процесса регистрации при неудачном входе (связь с `RegisterNewAccountUseCase`)
